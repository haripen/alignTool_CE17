...
def _select_otb4_c3d_edge_alignment(match: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if base_is_clearly_bad and best["basis"] != "late_c3d_raw_bridge":
        ...
        if best["basis"] not in ("late_c3d_raw_bridge", "general_skip_search"):
            # Channel 2 still could not be resolved. Try channel 3 as a
            # cross-machine fallback -- if it succeeds where channel 2
            # failed, that narrows the fault to channel 2's own cable run
            # (see _channel3_edge_alignment_fallback for the wiring reasoning).
            ...
            if ch3_fallback is not None:
                ...
                note = (
                    ...
                    "Vicon Lock Lab input feeding C3D -- both failing at once points more toward the "
                    "C3D/Vicon/Lock-Lab side (or a session-wide OTB4 clock/timing issue) than toward two "
