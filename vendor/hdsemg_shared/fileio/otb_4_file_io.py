...
def parse_otb4_tracks_xml(xml_file):
    ...
    for idx, tr_el in enumerate(track_info_elems):
        ...
        subtitle_str = _get_text_save(tr_el, "SubTitle", default="", do_strip= True)
        ...
        if desc_el is not None:
            ...
            sensor_type = _get_text_save(desc_el, "SensorType", default="", do_strip=True)
            ...
            logger.debug(f"    Name: {grid_name!r}, SensorType: {sensor_type!r}, NRow: {nrow_str!r}, NColumn: {ncol_str!r}, IED: {ied_str!r}")
        ...
        track_dict = {
            ...
            "DescriptionName": grid_name if desc_el is not None and grid_name else None,
            "SensorType": sensor_type if desc_el is not None and sensor_type else None,
        }
...
def _track_is_emg_grid(track):
    grid_info = track.get("GridInfo")
    if not grid_info:
        return False
    ied = grid_info.get("IED", 0)
    rows = grid_info.get("NRow", 0)
    cols = grid_info.get("NColumn", 0)
    electrodes = rows * cols
    return ied > 1 or electrodes > 4


def _track_base_conversion(track):
    nbits = track.get("ADC_Nbits", 0)
    gain = track.get("Gain", 1)
    adc_range = track.get("ADC_Range", 0)
    if nbits <= 0 or gain == 0:
        return 1.0
    return adc_range / (2 ** nbits) / gain


def _track_prefers_voltage_units(track):
...
def _track_display_unit(track):
    if _track_prefers_voltage_units(track):
        return "V"
    return None


def _track_conversion_factor(track):
...
def _build_track_descriptions(track):
    nchan = int(track.get("NumberOfChannels", 0))
...
def _iter_tracks_by_signal_file(track_info_list):
    tracks_by_file = {}
    for track in track_info_list:
        sig_file = track["SignalStreamPath"]
        tracks_by_file.setdefault(sig_file, []).append(track)
    return tracks_by_file


def _signal_file_channel_count(tracks):
    total = 0
    for track in tracks:
        start = int(track.get("AcquisitionChannel", 0))
        total = max(total, start + int(track.get("NumberOfChannels", 0)))
    return total


def _extract_track_block(raw_matrix, track):
    start = int(track.get("AcquisitionChannel", 0))
    nchan = int(track.get("NumberOfChannels", 0))
    end = start + nchan
    if start < 0 or end > raw_matrix.shape[0]:
        raise ValueError(
            f"Track channel range {start}:{end} is outside signal matrix with {raw_matrix.shape[0]} channels."
        )
    return raw_matrix[start:end, :]


def _load_signal_matrix(sig_path, total_channels, dtype):
    raw = np.fromfile(sig_path, dtype=dtype)
    n_values = raw.size
    logger.debug(f"Read {n_values} values from file {os.path.basename(sig_path)}")
    if total_channels <= 0 or n_values % total_channels != 0:
        raise ValueError(f"Invalid signal data length {n_values} for {total_channels} channels in {sig_path}")
    samples = n_values // total_channels
    matrix = raw[:samples * total_channels].reshape((total_channels, samples), order='F').astype(np.float64)
    logger.debug(f"Reshaped data to: {matrix.shape} (channels x samples)")
    return matrix
...
def read_novecento_plus(signals, track_info_list, trapezoidal_tracks, tmpdir):
    ...
    for tr in track_info_list:
        if _track_is_emg_grid(tr):
            grid_info = tr.get("GridInfo")
            last_valid_grid_pattern = f"HD{grid_info['IED']:02d}MM{grid_info['NRow']:02d}{grid_info['NColumn']:02d}"
    ...
    for sig_idx, sig_path in enumerate(signals):
        ...
        for tr_idx, matched_track in matched_tracks:
            ...
            logger.debug(f"    Channels: {n_ch}, AcquisitionChannel: {matched_track['AcquisitionChannel']}, IsControl: {matched_track['IsControl']}")
            ...
            block = _extract_track_block(all_data, matched_track)
            logger.debug(f"    Extracted block shape: {block.shape}")

            # Apply gain/scaling conversion
            conv = _track_conversion_factor(matched_track)
            ...
            is_emg_grid = _track_is_emg_grid(matched_track)
            ...
            descriptions.extend(_build_track_descriptions(matched_track))
    ...
    for sig_idx, sig_path in enumerate(signals):
        ...
        for tr_idx, matched_track in matched_tracks:
            ...
            if _track_is_emg_grid(matched_track):
                grid_info = matched_track["GridInfo"]
                grid_pattern = f"HD{grid_info['IED']:02d}MM{grid_info['NRow']:02d}{grid_info['NColumn']:02d}"
            ...
            # Increment channel_idx for all processed tracks.
            channel_idx += n_ch
...
def read_standard_otb4(signals, track_info_list, trapezoidal_tracks, tmpdir):
    ...
    tracks_by_file = _iter_tracks_by_signal_file(track_info_list)
    data_blocks = []
    descriptions = []
    fs_main = None
    ...
    for sig_path in signals:
        ...
        matched_tracks = tracks_by_file.get(sig_name, [])
        if not matched_tracks:
            logger.warning(f"No metadata tracks found for signal file {sig_name}, skipping")
            continue

        total_channels = _signal_file_channel_count(matched_tracks)
        logger.debug(f"Reading signal file {sig_name} with {len(matched_tracks)} tracks and {total_channels} channels")
        data_2d = _load_signal_matrix(sig_path, total_channels, np.int16)

        for tr_idx, trackinfo in enumerate(matched_tracks):
            block = _extract_track_block(data_2d, trackinfo)
            conv = _track_conversion_factor(trackinfo)
            block = block.astype(np.float64) * conv
            data_blocks.append(block)
            descriptions.extend(_build_track_descriptions(trackinfo))
            logger.debug(
                f"Track #{tr_idx} ({trackinfo['Device']} - {trackinfo['SubTitle']}): "
                f"acq={trackinfo['AcquisitionChannel']} nchan={trackinfo['NumberOfChannels']} conv={conv:.6f}"
            )

            if fs_main is None:
                fs_main = trackinfo["SamplingFrequency"]
            elif abs(fs_main - trackinfo["SamplingFrequency"]) > 1e-9:
                logger.warning(f"Inconsistent sampling freq: expected {fs_main}, got {trackinfo['SamplingFrequency']}")

    if data_blocks:
        lens = [block.shape[1] for block in data_blocks]
        if not all(length == lens[0] for length in lens):
            logger.warning(f"Different sample lengths among standard OTB4 tracks: {lens}. Picking min length.")
            min_len = min(lens)
            for idx in range(len(data_blocks)):
                data_blocks[idx] = data_blocks[idx][:, :min_len]
        final_data = np.vstack(data_blocks)
    else:
        final_data = np.zeros((0, 0))
    ...
    fs_main = fs_main if fs_main is not None else (track_info_list[0]["SamplingFrequency"] if track_info_list else 2000.0)

    logger.info(f"read_standard_otb4: Final data shape: {final_data.shape}, {len(descriptions)} descriptions, fs={fs_main} Hz")
    return final_data, descriptions, fs_main
